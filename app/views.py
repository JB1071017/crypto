...
def send_otp(request):
    if request.method == 'POST':
       # email = request.POST.get('email')
        email = request.session.get('email')  # Get email from the form
        ...
        try:
            ...
            from_email = 'nefsal003@gmail.com'
            ...
            # If email fails, still allow access
            request.session['email'] = email
            return redirect('about')
...
def verify_otp(request):
    if request.method == 'POST':
        ...
        if not stored_otp or entered_otp == stored_otp:
            return HttpResponse("<script>alert('Login successful'); window.location.href='/about/';</script>")
        else:   
